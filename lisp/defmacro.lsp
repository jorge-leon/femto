...
(defun dm-record-key(k)
  ...
  (cond
    ((eq k "\n")
     (insert-string "\n")
     (set-buffer dm-buffer)
     (insert-string "\n (insert-string \"\\n\")")
     (switch-to-buffer dm-obuf))
    (t
     (insert-string k)
     (set-buffer dm-buffer)
     (insert-string "\n (insert-string \"")
...
(defun dm-init()
  ...
  (setq dm-recording t)
  (setq dm-obuf (buffer-name))
  (set-buffer (get-buffer-create dm-buffer))
  (buffer-special-p dm-buffer t)
  (erase-buffer)
...
(defun dm-end-macro()
  (if-not dm-recording (message "macro recording is not in progress")
	  (setq dm-recording nil)
	  (setq dm-obuf (buffer-name))
	  (set-buffer dm-buffer)
	  (insert-string ")\n")
